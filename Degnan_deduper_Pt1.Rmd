...
*Step Five:* The sliding window will then slide down to the next read. In other words, the first entry in the numpy array will become the 31st line of the file (this can be tracked with a modulus, ie 31 % 30 = 1). The second entry in the array will then be compared to all the other lines, testing for any conflicts (Step Three) and if none found, being written to the "_deduped" file (Step Four). This continues for each line of the file until the end is reached. 
...
knitr::kable(input, caption = "Output Duplicate File", row.names = FALSE)
```
